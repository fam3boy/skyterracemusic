...
export default function DashboardPage() {
  ...
  return (
    <AdminLayout>
      <div className="mb-10">
        <h2 className="text-3xl font-bold text-hyundai-black tracking-tight">Dashboard</h2>
        <p className="text-hyundai-gray-500 mt-1">실시간 신청곡 현황 및 시스템 운영 기록</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        {/* Row 1: Key Metrics */}
        <div className="card-premium p-8 border-l-4 border-l-hyundai-black">
          <p className="text-xs font-black text-hyundai-gray-400 uppercase tracking-widest mb-2">Total Requests</p>
        ...
        <div className="card-premium p-8 border-l-4 border-l-yellow-400">
          <p className="text-xs font-black text-hyundai-gray-400 uppercase tracking-widest mb-2">Pending Review</p>
          <p className="text-5xl font-black text-hyundai-black tracking-tighter">{stats.pending}</p>
          <div className="mt-6 flex justify-end">
            <span className="text-[10px] font-black bg-yellow-400 text-white px-3 py-1 rounded-full tracking-tighter">ACTION REQUIRED</span>
          </div>
        </div>

        <div className="card-premium p-8 border-l-4 border-l-hyundai-emerald bg-hyundai-emerald/5">
          <p className="text-xs font-black text-hyundai-emerald uppercase tracking-widest mb-2">Weekly Preview</p>
             ...
             <button 
               onClick={async () => {
                 if(confirm('지금 즉시 주간 리포트를 발송하시겠습니까?')) {
                   const res = await fetch('/api/cron/weekly-mail?manual=true');
        ...
        <div className="card-premium p-8 border-l-4 border-l-hyundai-gold">
          <p className="text-xs font-black text-hyundai-gray-400 uppercase tracking-widest mb-2">Auto-Report Status</p>
          <div className="flex items-center gap-3">
            <div className={`w-3 h-3 rounded-full ${stats.lastMail?.status === 'success' ? 'bg-hyundai-emerald animate-pulse' : 'bg-red-500'}`}></div>
            <p className="text-2xl font-black text-hyundai-black uppercase tracking-tight">{stats.lastMail?.status || 'NO RECORD'}</p>
          </div>
          ...
          <div className="mt-6 flex justify-end">
             <span className={`text-[10px] font-black px-3 py-1 rounded-full uppercase ${stats.lastMail?.status === 'success' ? 'bg-hyundai-emerald/10 text-hyundai-emerald' : 'bg-red-50 text-red-500'}`}>
               {stats.lastMail?.status === 'success' ? 'System Stable' : 'Action Needed'}
             </span>
          </div>
        </div>
      </div>
          ...
          <div className="card-premium p-8">
            ...
            <div className="space-y-6">
              ...
              <div className="p-6 bg-hyundai-gray-50 rounded-[1.5rem] border border-hyundai-gray-100">
                ...
                <p className="text-sm text-hyundai-gray-500 leading-relaxed font-medium">매주 목요일 19:00 시스템에 의해 자동 집계되어 등록된 이메일로 발송됩니다. 필요한 경우 수동 발송 버튼을 사용하세요.</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="flex-grow">
            {loading ? (
                  ...
                  <div key={log.id} className="flex items-start gap-4 p-4 hover:bg-hyundai-gray-50 rounded-2xl transition-colors border border-transparent hover:border-hyundai-gray-100">
                    ...
                    <div className="flex-grow">
                      ...
                      <p className="text-[10px] text-hyundai-gray-400 font-bold uppercase tracking-widest mt-0.5">
                        {log.admin_name || 'System'} • {log.target_table} ({log.target_id?.slice(0, 8)})
                      </p>
                    </div>
                  </div>
                ))}
              ...
              <div className="text-center py-20">
                <p className="text-xs font-black text-hyundai-gray-300 uppercase tracking-widest leading-loose">No activities recorded yet.</p>
              </div>
            )}
          </div>
          <button className="mt-8 text-[10px] font-black text-hyundai-gray-400 hover:text-hyundai-black uppercase tracking-[0.2em] border-t border-hyundai-gray-100 pt-6">
